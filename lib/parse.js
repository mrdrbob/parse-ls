(function(){
  var inherits, toInput, inputNext, inputAtEof, inputCurrentLetter, pass, fail, withErrorMessage, simple, any, char, map, debug, $then, thenKeep, thenIgnore, thenConcat, thenNull, thenArrayConcat, $or, anyOf, many, times, atLeast, atLeastOnce, joinString, asArray, asObjectWithValue, thenSet, sequence, text, maybe, except, doUntil, expectEnd, end, delay, always, alwaysNew, parse, convertRuleToFunction, lineAndColumn;
  ...
  parse = curry$(function(rule, input){
    ...
    if ((res = rule(input)).success) {
      ...
      message = res.message;
      if (res.lastSuccess) {
        pos = lineAndColumn(res.lastSuccess);
